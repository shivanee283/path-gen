...
function Result({ paths }) {
  return (
    <div className="text-center text-gray-500 dark:text-gray-400 max-w-5xl w-full h-full flex flex-col gap-8 mx-auto">
      <h1 className="text-4xl font-extrabold text-center text-gray-900 dark:text-gray-100">
        Hurray!! Here's Your Learning Path 🎉
      </h1>
      <p className="text-lg text-center text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
        Below are the curated learning paths tailored for you. Start your journey to improve your financial literacy today!
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paths.map((path, index) => (
          <div
            key={index}
            className="flex flex-col gap-4 bg-gray-100 dark:bg-neutral-900 shadow-lg p-6 rounded-lg border border-gray-300 dark:border-neutral-800 hover:shadow-2xl transition-all duration-300"
          >
            ...
            <div className="flex flex-wrap gap-2">
              {path.skills.map((skill, index) => (
                <span
                  key={index}
                  className="inline-block bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-3 py-1 text-xs rounded-full"
                >
                  {skill}
                </span>
              ))}
